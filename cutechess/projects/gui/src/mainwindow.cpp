...
{
	m_openHookLensAct = new QAction(tr("HookLens"), this);
	m_openRoughGuardAct = new QAction(tr("RoughGuard"), this);
	...
	connect(m_openHookLensAct, &QAction::triggered, this, &MainWindow::openHookLensWindow);
	connect(m_openRoughGuardAct, &QAction::triggered, this, &MainWindow::openRoughGuardWindow);
...
void MainWindow::createMenus()
{
	...
	m_toolsMenu->addAction(m_openHookLensAct);
	m_toolsMenu->addAction(m_openRoughGuardAct);
...
void MainWindow::openHookLensWindow()
{
    auto* window = new HookLensWindow(this);
    window->setAttribute(Qt::WA_DeleteOnClose); // delete when closed
    connect(window, &HookLensWindow::fenStringReady, this, [this](const QString& fen) {
        if (!fen.isEmpty() && m_gameViewer && m_gameViewer->boardScene()) {
            // Set the FEN string on the board scene
            m_gameViewer->boardScene()->setFenString(fen);
            
            // Force a repaint to ensure the board is visually updated
            m_gameViewer->boardScene()->update();
            
            // Verify the board was set correctly
            Chess::Board* board = m_gameViewer->boardScene()->board();
            if (board) {
                QString currentFen = board->fenString();
                if (currentFen != fen) {
                    qWarning("Board FEN mismatch: expected '%s', got '%s'", 
                            qUtf8Printable(fen), qUtf8Printable(currentFen));
                }
            }
        }
    });
    connect(window, &HookLensWindow::bestMoveReady, this, [this](const QString& fromSquare, const QString& toSquare) {
        if (m_gameViewer && m_gameViewer->boardScene()) {
            // Create the move string in LAN format (e.g., "g4f6")
            QString moveString = fromSquare + toSquare;
            
            // Get the board from the scene
            Chess::Board* board = m_gameViewer->boardScene()->board();
            if (board) {
                // Ensure the board is in a valid state by checking if it has a valid FEN
                QString currentFen = board->fenString();
                if (currentFen.isEmpty()) {
                    QMessageBox::warning(this, "Board Error", 
                        "The board is not in a valid state. Please load an image first.");
                    return;
                }
                
                // Debug information
                QString sideToMove = board->sideToMove().toString();
                qDebug() << "Board state:";
                qDebug() << "  FEN:" << currentFen;
                qDebug() << "  Side to move:" << sideToMove;
                qDebug() << "  Attempting move:" << moveString;
                qDebug() << "  From square:" << fromSquare;
                qDebug() << "  To square:" << toSquare;
                
                // Check if the piece at the from square belongs to the side to move
                Chess::Square fromSq;
                if (fromSquare.length() == 2) {
                    int file = fromSquare[0].toLatin1() - 'a';
                    int rank = fromSquare[1].digitValue() - 1;
                    fromSq = Chess::Square(file, rank);
                }
                if (fromSq.isValid()) {
                    Chess::Piece piece = board->pieceAt(fromSq);
                    if (piece.isValid()) {
                        qDebug() << "  Piece at" << fromSquare << ":" << piece.side().toString() << piece.type();
                        if (piece.side() != board->sideToMove()) {
                            QMessageBox::warning(this, "Wrong Side", 
                                QString("The move %1 -> %2 is for %3, but it's %4's turn.\n\n"
                                       "Current FEN: %5").arg(fromSquare, toSquare, 
                                       piece.side().toString(), board->sideToMove().toString(), currentFen));
                            return;
                        }
                    } else {
                        qDebug() << "  No piece at" << fromSquare;
                    }
                }
                
                // Create the move from the string (automatically detects LAN format)
                Chess::Move move = board->moveFromString(moveString);
                if (!move.isNull() && board->isLegalMove(move)) {
                    // Make the move on the board scene
                    m_gameViewer->boardScene()->makeMove(move);
                } else {
                    // Provide more detailed error information
                    QString errorMsg = QString("The move %1 -> %2 is not legal in the current position.\n\n").arg(fromSquare, toSquare);
                    errorMsg += QString("Current FEN: %1\n").arg(currentFen);
                    errorMsg += QString("Side to move: %1\n").arg(sideToMove);
                    errorMsg += QString("Move string: %1").arg(moveString);
                    
                    // List legal moves for debugging
                    QVector<Chess::Move> legalMoves = board->legalMoves();
                    if (!legalMoves.isEmpty()) {
                        errorMsg += QString("\n\nLegal moves:");
                        for (const Chess::Move& legalMove : legalMoves) {
                            QString legalMoveStr = board->moveString(legalMove, Chess::Board::LongAlgebraic);
                            errorMsg += QString("\n  %1").arg(legalMoveStr);
                        }
                    }
                    
                    QMessageBox::warning(this, "Invalid Move", errorMsg);
                }
            } else {
                QMessageBox::warning(this, "Board Error", 
                    "No board available. Please load an image first.");
            }
        }
    });
    connect(window, &HookLensWindow::resetRequested, this, [this]() {
        if (m_gameViewer && m_gameViewer->boardScene() && m_gameViewer->boardScene()->board()) {
            // Reset the board to the default starting position
            m_gameViewer->boardScene()->board()->reset();
            m_gameViewer->boardScene()->populate();
        }
    });
    window->show();
}
void MainWindow::openRoughGuardWindow()
{
	auto* window = new RoughGuardWindow(this);
	window->setAttribute(Qt::WA_DeleteOnClose); // delete when closed
	window->show();
}
