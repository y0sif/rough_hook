...
use crate::data_and_model::model::Cnn;
use crate::input_data_handling::board_square_extracting::*;
use crate::data_and_model::inference::{self, ModelEnum};
pub fn get_fen_string_from<B: Backend>(img_data : Vec<u8> , model : Cnn<B>, device: &B::Device)->String
where
...

{
    ...
    for i in 0..64{
        ...
        let piece = map[&inference::infer_model::<B>(&model, device.clone(), image.to_vec())];
        if piece == "e" {
        empty_squares += 1;
        }else{
            if empty_squares != 0 {
