...
pub fn get_fen_string_from(board_image_path : &str , model : ModelEnum<Wgpu>)->String{
    ...
    for pos in start{
        ...
        for _ in 0..8{
            ...
            let piece = map[&inference::infer_model::<Wgpu>(&model, WgpuDevice::default(), image.to_vec())];
            if piece == "e" {
                empty_squares += 1;
            }else{
                if empty_squares != 0 {
