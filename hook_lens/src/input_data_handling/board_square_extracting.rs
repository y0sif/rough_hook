...
fn prepare_image_on_template_image(target_image: &str) -> Result<Mat, opencv::Error> {
    ...
    // Load second image
    let img2_reader =
        BufReader::new(File::open(second_image_path).expect("Cannot open second image"));
    let img2_format = image::guess_format(&std::fs::read(second_image_path).unwrap())
        .expect("Cannot detect format");
    let img2 = image::load(img2_reader, img2_format).expect("Cannot load second image");
    ...
    let mat = binding.reshape(4, height as i32)?; // 4 channels for RGBA8
    let mut converted_mat = Mat::default();
    imgproc::cvt_color(&mat, &mut converted_mat, imgproc::COLOR_RGBA2BGRA, 0)?;
