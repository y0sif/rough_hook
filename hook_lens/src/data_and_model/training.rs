...
use super::model::{CustomCnn, CustomKanCnn, CustomModel};
...
impl<B: AutodiffBackend> TrainStep<ChessBoardBatch<B>, ClassificationOutput<B>> for CustomKanCnn<B> {
    fn step(&self, batch: ChessBoardBatch<B>) -> TrainOutput<ClassificationOutput<B>> {
        let item = self.forward_classification(batch.images, batch.targets);
        TrainOutput::new(self, item.loss.backward(), item)
    }
...
impl<B: Backend> ValidStep<ChessBoardBatch<B>, ClassificationOutput<B>> for CustomKanCnn<B> {
    fn step(&self, batch: ChessBoardBatch<B>) -> ClassificationOutput<B> {
        self.forward_classification(batch.images, batch.targets)
    }
}
...
pub fn train_kan<B: AutodiffBackend>(model: CustomKanCnn<B>, config: TrainingConfig, device: B::Device) 
where 
