    ...
    B::FloatElem: ndarray_linalg::Scalar + ndarray_linalg::Lapack,
{
    ...
    fn forward(&self, x: Tensor<B, 4>) -> Tensor<B, 2> {
        match self {
            ModelEnum::Cnn(model) => model.forward(x.clone()),
    ...
    B::FloatElem: ndarray_linalg::Scalar + ndarray_linalg::Lapack,
{
    ...
    println!(" model : {:?}" , model_object);

    let model = match model_object {
        ModelEnum::kan_cnn_256(kan_cnn_256) => kan_cnn_256 , 
        _=>panic!("")
    };
    ...
    let mode12 = model.clone();
    ModelEnum::kan_cnn_256(mode12)
}
