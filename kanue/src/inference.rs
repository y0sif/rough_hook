...
use crate::{data::{ChessPositionBatcher, ChessPositionItem}, training::KanTrainingConfig};
...
{    
    ...
    let model = config.model.init::<B>(&config.kan_options, &device).load_record(record);
